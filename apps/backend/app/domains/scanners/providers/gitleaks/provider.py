"""Gitleaks analysis provider — secret detection scanner adapter.

Implements the ``AnalysisProvider`` port from the analysis domain, bridging
the analysis orchestrator to the Gitleaks CLI.  Scanner-specific logic stays
...
- ``GitleaksResultParser`` normalizes JSON output into ``ScannerFinding``
  objects with **mandatory secret redaction**

The orchestrator calls ``execute(context)`` — it never knows about Gitleaks.

SECURITY: The ``Match`` field from Gitleaks (containing the actual secret)
is DISCARDED during parsing.  Only safe metadata is persisted.
"""
...
from app.domains.scanners.providers.gitleaks.parser import GitleaksResultParser
from app.domains.scanners.providers.gitleaks.runner import GitleaksRunner

logger = logging.getLogger(__name__)


class GitleaksProvider:
    """Gitleaks secret detection provider.

    Implements ``AnalysisProvider`` protocol so the orchestrator can dispatch
    it as a drop-in scanner alongside Trivy.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._runner = GitleaksRunner(
            executable=executable,
            timeout_seconds=timeout_seconds,
        )
        self._parser = GitleaksResultParser(
    ...
    def supports(self, repository: Any) -> bool:
        """Gitleaks can scan any non-archived, non-disabled repository."""
    ...
    async def execute(self, context: AnalysisExecutionContext) -> None:
        """Execute a Gitleaks secret scan with repository acquisition.

        Pipeline:
        1. Validate Gitleaks is available
        2. Acquire repository via GitHub API
        3. Run Gitleaks detect on the cloned checkout
        4. Parse JSON output (secrets REDACTED)
        ...
        # Validate Gitleaks is available
        if not GitleaksRunner.is_available(self._executable):
            raise ProviderError(
                f"Gitleaks is not installed or not available in PATH "
                f"(executable: {self._executable}). "
                "Install Gitleaks: https://github.com/gitleaks/gitleaks",
                code="scanner_unavailable",
            )

        version = GitleaksRunner.get_version(self._executable)
        logger.info(
            "Starting Gitleaks scan for %s (version=%s, run=%s)",
            context.full_name,
            version,
            context.run_id,
        )
        ...
        try:
            ...
            # Execute Gitleaks on the cloned checkout
            repo_path = workspace.repo_path
            result = await self._runner.run_detect(
                repo_path,
            ...
            if not result.success:
                # Exit code 1 = secrets found (NOT a failure).
                # Exit code 2+ = execution error.
                # SECURITY: Never include raw stderr — it may contain
                # discovered secrets or sensitive repository content.
                raise ProviderError(
                    f"Gitleaks scan failed with exit code {result.exit_code}.",
                    code="scanner_execution_failed",
                )

            # Parse the output — secrets are REDACTED by the parser.
            ...
            logger.info(
                "Gitleaks scan complete: %d secret findings for %s (run=%s)",
                len(findings),
                context.full_name,
                context.run_id,
            )
